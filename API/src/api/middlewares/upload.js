...
const uploadFileStorage = multer.diskStorage({
  ...
  filename: function (req, file, callback) {
    const filename = `${moment(new Date()).format("YYYYMMDDHHmmss") + file.originalname}`
    callback(null, filename);
  },
});
const uploadIconAwardStorage = multer.diskStorage({
  destination: (req, file, callback) => {
    callback(null, "./" + process.env.UPLOADED_FOLDER + "/award");
  },
  filename: function (req, file, callback) {
    const filename = `${moment(new Date()).format("YYYYMMDDHHmmss") + file.originalname}`
    callback(null, filename);
...
const uploadIconAward = multer({
  storage: uploadIconAwardStorage,
  fileFilter: (req, file, cb) => {
    if (!/[^\u0000-\u00ff]/.test(file.originalname)) {
      file.originalname = Buffer.from(file.originalname, "latin1").toString(
        "utf8"
